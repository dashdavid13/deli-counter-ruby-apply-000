...
def line(katz_deli)
  if katz_deli.empty?
    ...
    katz_deli.each.with_index(0) do |name, number| #index number will start with 1
      current_line << " #{number+1}. #{name}" # position in the line with name has been added to the end the variable current_line 
    end
